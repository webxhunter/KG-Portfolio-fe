...
import { Camera, Upload, X, Edit, Trash2, AlertTriangle, ImageIcon } from 'lucide-react';
import Image from 'next/image';

const ImageCard = ({ media, onEdit, onDelete, apiUrl }) => (
  <div className="relative group bg-slate-900 rounded-lg overflow-hidden border border-slate-700">
    ...
    <div className="absolute top-2 right-2 flex gap-2  transition-opacity">
      ...
      <button onClick={() => onDelete(media.id)} className="p-2 bg-red-600/80 hover:bg-red-700 rounded-full text-white" title="Delete image">
        <Trash2 size={16} />
      </button>
    </div>
  </div>
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <div className="bg-[#1C2135] border border-slate-700/50 rounded-xl w-full max-w-lg p-6 space-y-4 relative">
        ...
        <div className="relative h-56 bg-slate-900 rounded-lg overflow-hidden border border-slate-700">
          <Image
            src={previewUrl || `${apiUrl}/uploads/${media.image_url}`}
            alt=""
            fill
            className="object-cover"
          />
...
const GalleryAdminPanel = () => {
  ...
  // ✅ Added: location state (from ImageUpload)
  ...
  const handleSubmit = async () => {
    ...
    // ✅ Changed: 'file' → 'image' (aligned with ImageUpload)
    formData.append('image', file);
    formData.append('category', serviceName);
    // ✅ Changed: hardcoded 'gallery' → dynamic location state
  ...
  const handleUpdate = async (id, updatedFile, previousLocation, categoryFromParams) => {
    if (!updatedFile) { toast.warn("Please select a new image to update."); return; }
    const formData = new FormData();
    // ✅ Changed: 'file' → 'image' (aligned with ImageUpload)
    formData.append('image', updatedFile);
    formData.append('location', previousLocation);
    formData.append('category', categoryFromParams);
    try {
  ...
  return (
    <>
          ...
          <div className="w-full rounded-xl border border-slate-700/50 bg-[#1C2135] p-8 shadow-2xl shadow-blue-500/10">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              <div className="space-y-6">
                {/* ✅ Added: Target Location selector (from ImageUpload) */}
                <div>
                  <label className="block mb-2 text-sm font-semibold text-slate-300">Target Location</label>
                  <select
                    value={location}
                    onChange={(e) => setLocation(e.target.value)}
                    className="w-full px-4 py-2.5 bg-slate-900 border border-slate-700 rounded-md focus:ring-2 focus:ring-blue-500 transition"
                  >
            ...
            <div className="w-full rounded-xl border border-slate-700/50 bg-[#1C2135] p-8">
              ...
              {hero && (
                <div className="mb-8">
                  ...
                  <ImageCard media={hero} onEdit={handleEditClick} onDelete={handleDeleteClick} apiUrl={API_URL} />
                </div>
              )}
              {images.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {images.map((image) => (
                      <ImageCard key={image.id} media={image} onEdit={handleEditClick} onDelete={handleDeleteClick} apiUrl={API_URL} />
                    ))}
                  </div>
                </div>
              )}
      ...
      <EditModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} media={editingMedia} onSave={handleUpdate} apiUrl={API_URL} serviceName={serviceName} />
