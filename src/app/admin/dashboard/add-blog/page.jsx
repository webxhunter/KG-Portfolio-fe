...
const AddBlog = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/blogs/${editId}`, {
        method: "PUT",
        body: data,
      });
      ...
      <div className="bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 border border-blue-500/30">
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors shadow-[0_0_12px_rgba(239,68,68,0.5)]"
          >
            Delete
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  type="date"
                  name="date"
                  value={editId ? editForm.date : form.date}
                  onChange={(e) =>
                    editId
                  ...
                  required
                  className="w-full bg-gray-900/70 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label
                ...
                <input
                  ...
                  type="text"
                  name="question"
                  placeholder="Blog title or question"
                  value={editId ? editForm.question : form.question}
                  onChange={(e) =>
                    editId
            ...
            <div>
              ...
              <textarea
                ...
                value={editId ? editForm.answer : form.answer}
                onChange={(e) =>
                  editId
                ...
                required
                rows={5}
                className="w-full bg-gray-900/70 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y"
              />
            </div>
            <div>
              ...
              <div
                onClick={() =>
                  (editId ? editImageRef : addImageRef).current?.click()
                }
                className="relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-600 rounded-lg cursor-pointer hover:border-blue-500 hover:bg-gray-800/60 transition-all bg-cover bg-center"
                style={{
                  backgroundImage: `url(${editId ? editPreview : preview})`,
                }}
              >
                {!(editId ? editPreview : preview) && (
                ...
                <input
                  ref={editId ? editImageRef : addImageRef}
                  type="file"
                  className="hidden"
                  accept="image/*"
                  onChange={(e) => handleFileChange(e, editId ? "edit" : "add")}
                />
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-4 pt-2">
              <button
                ...
                className="w-full sm:w-auto flex-grow px-6 py-3 rounded-lg bg-blue-600 text-white font-bold text-lg hover:bg-blue-700 transition-all duration-300 disabled:bg-blue-800 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(59,130,246,0.5)]"
              >
              ...
              {editId && (
                <button
                  type="button"
                  onClick={cancelEdit}
                  className="w-full sm:w-auto px-6 py-3 rounded-lg bg-gray-700 text-white font-semibold hover:bg-gray-600 transition-colors"
                >
                  Cancel Edit
                </button>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map((blog) => (
            <div
              key={blog.id}
              className="bg-gray-800/50 backdrop-blur-sm border border-blue-500/20 rounded-2xl shadow-lg shadow-blue-500/10 flex flex-col overflow-hidden"
            >
              <img
